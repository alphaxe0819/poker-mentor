...
- [?] **T-097** | Pipeline | **Pipeline v2 改造：seed + worker + progress schema + RPC 四合一對齊 `gto_solutions`**
  - branch：`wip/T097-pipeline-v2`（from `origin/dev@6fd9cc9`）
  - 機器：家裡 wip1 worktree
  - 4 commit（依用戶建議拆）：
    1. `ea8cff9` feat(T-097): migration 20260425 gto_batch_progress v2 schema
    2. `503589f` feat(T-097): scenarios.mjs 加 gametype / depth_bb / preflop_actions
    3. `e2dd8c4` feat(T-097): seed-batches.mjs 改產出 gametype-based seed
    4. `8f247cf` feat(T-097): batch-worker 改造對齊 gto_solutions + path-aware role
  - 改動檔（5 檔）：
    - `supabase/migrations/20260425-gto-batch-progress-v2.sql` 新檔 +212 行（DROP 舊 + CREATE 新 gto_batch_progress + RPC v2 + 6 條驗證查詢 a-f）
    - `scripts/gto-pipeline/scenarios.mjs` +36/-6（HU_SCENARIOS 7 entry 加 gametype/preflop_actions + HU_SRP_BY_STACK / getScenarioByGametype helper）
    - `scripts/gto-pipeline/seed-batches.mjs` +18/-5（row shape 改 v2 鍵 + onConflict 對齊）
    - `scripts/gto-pipeline/batch-worker.mjs` +220/-114（main 大改：pathToRole→encodeAction+advancePot / extractTurnRiverNodes→extractSpots / uploadRows→uploadSpots to gto_solutions / main loop 吃 v2 batch shape + --gametype-filter/--depth-filter RPC 參數）
  - 靜態驗證：`node --check` 3 個 .mjs + SQL 檢查（無外部執行，但檔頭 idempotent claim）通過
  - **F 驗證階段 BLOCKED**：migration 未部署測試 Supabase
    - 執行者紀律：**不自己貼 Dashboard**（用戶 2026-04-23 明確指示，DB migration 走大腦 merge 時產貼碼指令）
    - 需大腦 merge 後產貼碼指令給用戶 → 用戶貼測試 Supabase + 驗證查詢 (a)-(f) → 執行者（任一 session）重 seed + 跑 1 個 HU 25bb SRP batch 驗 pipeline 端到端
  - ⚠ **已知限制：TexasSolver v0.2.0 無 EV dump**
    - 實測 JSON 只含 {actions, childrens, dealcards, deal_number, node_type, player, strategy}，無 ev / equity / value 欄位
    - CLI 無 `set_dump_ev` 指令，sample_parameters 也沒
    - node_data.hands items 結構只有 `[{action, freq}]`（不含 ev）
    - `solver_config.ev_available: false` 顯式標示，下游 retrieval 可據此判斷
    - **長期解**：(a) GTOW API pull source='gtow_api' 帶 EV；(b) patch TexasSolver source 加 dump_ev；(c) 接受 freq-only（當前 retrieval 只用 freq，教練 grounding 也只靠 freq）
    - schema jsonb 彈性保留，未來補 EV 不需動 schema
  - **預期驗收結果**（等用戶貼 migration 後跑 1 個 batch 驗）：
    - HU 25bb SRP 25bb turn batch → 產 X 個 spots（X 預期遠少於 T-045 的 594 roles，因每 spot 現在 = 1 row 而非 169 rows/role）
    - dedup ratio ~1x（path-aware PK，每 spot 唯一 — 對比 T-045 舊 4-role 桶的 3.2x dup）
    - `gto_solutions` 新增 X rows，每 row 的 `node_data.hands` 含該 spot 169 hand classes 的 [{action, freq}]
    - `solver_config` 記 exploitability / iterations / solver_version / ev_available:false
  - **後續依賴**：
    - T-098（retrieval lib 改造）依賴此 pipeline 能產新表 rows
    - T-094（13bb marathon）依賴此對齊，才能 marathon 產的資料直接進 gto_solutions 免 migrate
